...
/**
 * persistant version of slist, tries to load
 * from filename, if possibe, else creates an
 * empty slist
 */
...
/**
 * write a slist to the given filename so it can be loaded
 * with slist_create_persist ()
 */
void slist_persist(char* filename, struct slist *l);
...
/**
 * searches the slist and returns the snode structure
 * if the text field matches.
 * @return pointer to an snode on match or NULL on failure
 */
